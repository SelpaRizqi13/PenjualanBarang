            ...
            <div class="col-xl-12 mb-5 mb-xl-0">
                <div class="card shadow p-4">
                    <div class="card-header border-0">
                        <form method="POST" action="{{ url('jenisbarang/' . $model->id) }}">
                            @csrf
                            <input type="hidden" name="_method" value="PATCH">
                            <div class="form-row">
                                <div class="form-group col-md-12">
                                    <label for="contoh2">Nama Jenis Barang</label>
                                    <input type="text" class="form-control" value="{{ $model->nama_jenisbarang }}" name="nama_jenisbarang" id="nama_jenisbarang" placeholder="Nama Jenis Barang">
                                </div>
                                <div class="form-group col-md-12">
                                    <label for="contoh2">Deskripsi Jenis Barang</label>
                                    <textarea name="deskripsi" id="deskripsi" value="{{ $model->deskripsi }}" cols="30" rows="5" class="form-control">{{ $model->deskripsi }}</textarea>
                                </div>
                            </div>
